...
export function EventsScreen({ onNavigate }: EventsScreenProps) {
  ...
  const [dynamicLocations, setDynamicLocations] = useState<string[]>(["all"]);
  ...
  const EventData = async () => {
    const EvData = await GETFunction(apiroute.eventurl);

    // Map events

    const mappedEvents = EvData.userlogData.map((ev: any) => ({
      id: ev.ID,
      title: ev.EventActivityName,
      image: ev.ImageURL,
      date: ev.EventDate,
      location: `${ev.BuildingName}${ev.RoomName ? " " + ev.RoomName : ""}`,
      category: ev.EventType,
      attendees: 0,
      price: "Free",
      isBookmarked: false,
      isRSVPd: ev.RSVPStatus === 1,
    ...
    // Create dynamic locations list
    const locations = [
      "all",
      ...Array.from(
        new Set(
          EvData.userlogData.map(
            (ev: any) =>
              `${ev.BuildingName}${ev.RoomName ? " " + ev.RoomName : ""}`
          )
        )
      ),
    ];

    setDynamicLocations(locations as string[]);
  ...
  const handleRSVP = async (eventId: string) => {
    ...
    const AddRSVP = await GETFunction(apiroute.joineventurl, parms);
    await EventData();
  };
